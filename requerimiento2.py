...
# Leer el archivo requerimiento2.py para verificar su contenido y asegurarse de que define la función `estadisticas_generales`
base_dir = os.path.dirname(os.path.abspath(__file__))
requerimiento2_path = os.path.join(base_dir, "requerimiento2.py")
with open(requerimiento2_path, "r", encoding="utf-8") as f:
    requerimiento2_code = f.read()

# Verificamos si contiene una función llamada estadisticas_generales
"def estadisticas_generales" in requerimiento2_code
...
def estadisticas_generales(data):
    """
    Genera estadísticas generales sobre los datos de publicaciones.
    """
    # Contar la cantidad total de artículos
    total_articulos = len(data)

    # Contar la cantidad de autores únicos
    autores_unicos = set()
    for articulo in data:
        autores = articulo.get("author", "").split(",")
        autores_unicos.update(autor.strip() for autor in autores)

    total_autores = len(autores_unicos)

    # Contar la cantidad de journals únicos
    journals_unicos = set(articulo.get("journal", "Unknown") for articulo in data)
    total_journals = len(journals_unicos)

    # Contar la cantidad de publishers únicos
    publishers_unicos = set(articulo.get("publisher", "Unknown") for articulo in data)
    total_publishers = len(publishers_unicos)
